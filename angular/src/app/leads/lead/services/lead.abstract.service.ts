...
import type { GetLeadsInput, LeadDto } from '../../../proxy/leads/models';
import { LeadService } from '../../../proxy/leads/lead.service';

export abstract class AbstractLeadViewService {
  protected readonly proxyService = inject(LeadService);
  ...
  data: PagedResultDto<LeadDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<LeadDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetLeadsInput;
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<LeadDto>) => {
      this.data = list;
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'Lead.xlsx');
      });
  }
}
