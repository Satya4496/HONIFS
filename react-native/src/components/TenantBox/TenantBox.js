 ...
 */
function TenantBox({ tenant = {}, setTenant, showTenantSelection, toggleTenantSelection }) {
      ...
      {showTenantSelection ? (
        <Box
          ...
          <FormControl my="2" width={350}>
            <FormControl.Label>{i18n.t('AbpUiMultiTenancy::Name')}</FormControl.Label>
            <Input autoCapitalize="none" value={tenantName} onChangeText={setTenantName} />
          </FormControl>
          <Text style={styles.hint}>{i18n.t('AbpUiMultiTenancy::SwitchTenantHint')}</Text>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
            <Button style={styles.button} onPress={() => toggleTenantSelection()} variant="outline">
